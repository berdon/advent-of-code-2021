...
`;
    async function main() {
        ...
        var result = visit(data, [0,0], [data[0].length - 1, data.length-1])
        var elapsed = (performance.now() - startTime).toFixed(2)
        display(data, result.path)
        console.log(`Part 1: Total risk is ${result.risk} (${elapsed} ms)`)
        ...
        var result = visit(largerGraph, [0,0], [largerGraph[0].length - 1, largerGraph.length-1])
        var elapsed = (performance.now() - startTime).toFixed(2)
        // display(largerGraph, result.path) // Adds considerable time to execution
        console.log(`Part 2: Total risk is ${result.risk} (${elapsed} ms)`)
    }
    ...
    function visit(graph: number[][], start: number[], end: number[]): { distances: Map<string, number>, previous: Map<string, string|null>, path: {key: string, risk: number}[], risk: number } {
